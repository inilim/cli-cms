...
- Репозитории: реализация Repository паттерна для работы с данными
...
## Key Array Naming Conventions (PHP)
- Ключи массивов должны быть **SnakeCase** с нижним регистром.
...
Статический анализ выдает ошибки из-за библиотеки inilim/tools, дело в том что методы из библиотеки вызываются через магический метод __callStatic. Анализатор не видит stub классы внутри библиотеки, если такие ошибки будут, то добавляй типы через комментарии, пример ниже
```php
