...
- Обновлены сущности для поддержки динамических свойств через DynamicEntityAbstract
- Реализована поддержка связей между сущностями в RecordRepository
- Добавлены новые методы в RecordRepository для получения записей с категориями за один запрос
- Обновлены контроллеры для работы с динамическими сущностями и связями
- Добавлен сервис RecordBlockProcessingService для обработки блоков в записях
- Оптимизировано использование DI-контейнера в сервисах - теперь можно использовать сквозной вызов \DI() без необходимости внедрения зависимостей через конструктор
